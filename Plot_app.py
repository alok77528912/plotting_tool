...
class ACPlotApp(MDApp):
    ...
    def set_textfield_style(self):
        for key in self.root.ids:
            widget = self.root.ids[key]
            if isinstance(widget, MDTextField):
                widget.theme_text_color = "Custom"
                widget.text_color = (0, 0, 0, 1)
                widget.hint_text_color_normal = (0, 0, 0, 1)
                widget.hint_text_color_focus = (0, 0, 0, 1)
                widget.font_style = "Subtitle1"
    ...
    def on_start(self):
        for widget in self.root.walk():
            if isinstance(widget, MDTextField):
                widget.theme_text_color = "Custom"
                widget.text_color = (0, 0, 0, 1)
                widget.hint_text_color_normal = (0.5, 0.5, 0.5, 1)
                widget.hint_text_color_focus = (0.5, 0.5, 0.5, 1)
                widget.font_style = "H6"
    ...
    def confirm_rename_test_case(self, old_name, new_name):
        self.dialog.dismiss()
        if not new_name or new_name == old_name or new_name in self.test_cases:
            return

        # Preserve order using a new dictionary
        new_test_cases = {}
        for name in self.test_cases:
            if name == old_name:
                new_test_cases[new_name] = self.test_cases[old_name]
            else:
                new_test_cases[name] = self.test_cases[name]
    ...
    def update_test_case_data(self, instance, value):
        ...
        cast_fields = {
            ...
            "line_y": str, "limit_label": str, "x_key": str, "y_key": str, "extra_key": str, "plot_column": str,
            "x_range": str, "y_range": str, "xy": int, "x": int, "y": int, "title_name": str, "file_name": str
        }
        ...
        #     # ✅ Only update file_len in all test cases
        #     for name in self.test_cases:
        #         self.test_cases[name]["file_len"] = parsed_value
        # else:
            # ✅ Update only the selected test case
    ...
    def run_test(self):
        """Runs all enabled test cases with a spinner replacing the Play button."""
        enabled_tests = [
            name for name, data in self.test_cases.items()
            if data.get("status", "Disabled") == "Enabled"
        ]
        ...
        self.root.ids.app_bar.left_action_items = []

        # ✅ Create a black spinner
        self.spinner = MDSpinner(
            size_hint=(None, None),
            size=(30, 30),  # Size similar to play icon
            pos_hint={"center_y": 0.5},
            color=(0, 0, 0, 1)  # Black spinner color
        )
        self.root.ids.app_bar.ids.left_actions.add_widget(self.spinner)
        ...
        def run_all_tests(*args):
            for test_name in enabled_tests:
                ...
                try:
                    self.load_test_details_by_name(test_name)
                    test_data = self.collect_input_fields()
                    self.run_test_case(test_name, test_data)
                except Exception as e:
                    print(f"❌ Error running test {test_name}: {e}")
                    errors.append(f"{test_name} ({e})")

            # ✅ After tests finish
            self.root.ids.app_bar.ids.left_actions.remove_widget(self.spinner)
            self.root.ids.app_bar.left_action_items = [["play", lambda x: self.run_test()]]
        ...
        Clock.schedule_once(run_all_tests, 0.2)
